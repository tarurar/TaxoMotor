...
BEGIN
	...
	SELECT 
		[T].[FieldName],
		CASE 
			WHEN [T].[VALUE] IS NULL THEN 
				CASE
					WHEN [T].[HASKEY] = 1 THEN 'NULL'
					ELSE [T].[STATEMENT]
				END
			ELSE [T].[VALUE]
		END AS [EXPRESSION]
	FROM (
		SELECT [F].[FIELDNAME],
			'T.REF.value(''' + [F].[FIELDNAME] + '[1]'', ''' + [F].[DATATYPE] + CASE [F].[DATATYPE]
				WHEN 'NVARCHAR'
			...
			END AS [VALUE],
			CASE 
				WHEN [V].[KEY] IS NULL THEN 0
				ELSE 1
			END AS HASKEY
		FROM [DBO].[fn_GetFieldsForTable](@TARGETTABLESCHEMA, @TARGETTABLENAME) F
		LEFT JOIN @VALUES V ON [F].[FIELDNAME] = [V].[KEY]
		WHERE 
			COLUMNPROPERTY(OBJECT_ID(@TARGETTABLEFULLNAME), [F].[FIELDNAME], 'IsIdentity') != 1 AND
			COLUMNPROPERTY(OBJECT_ID(@TARGETTABLEFULLNAME), [F].[FIELDNAME], 'IsComputed') != 1
		) T
	...
	SET @STATEMENT =	'SET NOCOUNT ON;' + 
						'INSERT INTO [' + @TARGETTABLESCHEMA + '].[' + @TARGETTABLENAME + '] (' + @FIELDLIST + ')' + 
						'SELECT ' + @SELECT + ' ' + 'FROM ' + 
							CASE LEFT(@SOURCEXMLPARAMNAME, 1)
								WHEN '@' THEN @SOURCEXMLPARAMNAME
								ELSE '@' + @SOURCEXMLPARAMNAME
							END + '.nodes(''' + @SOURCEXMLPATH + ''') T(REF);' +
						'SELECT ' + 
							CASE LEFT(@IDOUTPARAMNAME, 1)
								WHEN '@' THEN @IDOUTPARAMNAME
								ELSE '@' + @IDOUTPARAMNAME
							END + ' = SCOPE_IDENTITY();';
